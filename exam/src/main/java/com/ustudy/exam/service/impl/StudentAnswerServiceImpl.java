...
public class StudentAnswerServiceImpl implements StudentAnswerService {
			...
			if (null != data.get("sipisModelList")) {
				...
				if(studentPapers.size()>0){
				    ...
				    for (int i = 0; i < studentPapers.size(); i++) {
				        ...
				        if(answers.size()>0){
				            answerDaoImpl.insertStudentObjectAnswers(getAnswers(refAnswers, quesAnswers, multipleScoreSets, answers));
				        }
				    }
				}
	 ...
	 * @return
	 */
	private List<StudentObjectAnswer> getAnswers(Map<Integer, RefAnswer> refAnswers, Map<Long, QuesAnswer> quesAnswers, Map<Integer, MultipleScoreSet> multipleScoreSets, List<StudentObjectAnswer> answers){
        
        for (StudentObjectAnswer answer : answers) {
            RefAnswer refAnswer = refAnswers.get(answer.getQuesno());
            if(null != refAnswer && null != answer.getAnswer() && null != refAnswer.getAnswer()){
